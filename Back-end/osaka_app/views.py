...
from osaka_app import filtered_words
...
class GptOb:                 #정적 클래스를 사용하여 유저마다 유효한 GPT 질문&대답 리스트를 유지 및 관리
    ...
    @staticmethod
    def max_tokens_prevention(user_key): #지역관련 주요 데이터를 제외한 오래된 질문&응답 객체 2세트 삭제
...
def answer_gpt(request): #사용자가 질문창으로 질문함
    ...
    denial_list = [*filtered_words.negative_list, *filtered_words.food_list]  #예외 단어 리스트
    denial_chek = False                               #예외 단어 포함여부
    for keyword in [*keyword_list]:
        if keyword not in user_question:
            keyword_list.remove(keyword)
    #DB데이터 키워드 한개라도 포함하면 그때 예외 단어 포함 여부 검사
    if len(keyword_list) != 0:
        for denial in denial_list:
            if denial in user_question:
                denial_chek = True
                break
    #DB데이터와 GPT데이터 응답구분
    ...
    try:
        ...
        #max_tokens방지 시작
        #해당 유저키의 질문응답리스트에서 오래된 데이터 삭제 후 API재요청하여 max_tokens방지
        GptOb.max_tokens_prevention(request.GET["user_key"])
        messages = GptOb.getter_userlist(request.GET["user_key"])
        #GPT API 재요청
        assistant_content = gpt_api_request(messages, request.GET["user_key"])
        #max_tokens방지 완료
